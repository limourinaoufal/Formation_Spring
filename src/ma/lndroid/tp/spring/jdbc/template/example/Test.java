...
public class Test {
	private static final String PATH = "ma/lndroid/tp/spring/jdbc/template/example/applicationContext.xml";
	public static void main(String[] args) {
		...
		for(int i = 1;i<5;i++){
			Employee e = applicationContext.getBean("emp",Employee.class);
				...
				e.setName("LINA:"+i);
				e.setSalary(i*1000);
			int resI = empDao.saveEmployee(e);
			System.out.println("\t-Insert data status: {"+(resI==1?"OK":"KO")+"} -> "+e);
		...
		System.out.println("\t-UPDATE data status : {"+(resU==1?"OK":"KO")+"} - EMP ID = 2 To -> "+e2);
		...
		System.out.println("\t-DELETE data status: {"+(resD==1?"OK":"KO")+"} : EMPLOYEE ID = 3  ");
